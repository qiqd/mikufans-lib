...
### 请以【东京电视台、NHK、AT-X、WOWOW、AniDB、MyAnimeList、日本动画年鉴及官方等放送记录】为基础，完整、系统、无删减地整理整理 2000年 在日本放送的动画番剧列表(不排除续作，重制版，ova等等，反正只要是开播就算）,不能选取部分作品作为代表展示，不能找借口不回答，要求完整的返回,此外为保证结果完整准确(防止上下文token限制而导致的结果不完整或者压缩字段)，按照多次返回数据(比如第一批，第二批等等)，每次返回15条数据，

**返回这15条数据之后询问我是否继续下一个批次，并告知剩下没有汇报的番剧的数量**，直到这一年结束为止,这一年结束之后提醒我这一年的番剧放送已经结束。

**注意放送时间应该严格按照年份来划分,并且只返回该年在日本放送的番剧，不能返回其他年份放送的番剧**，返回的格式如下(
注意每个字段的类型,不能有误)：
